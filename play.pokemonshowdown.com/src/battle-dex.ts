...
//
const skylanderSpecies = ["Spyro", "Double Trouble", "Wrecking Ball", "Voodood",
	 "Bash", "Terrafin", "Prism Break", "Dino-Rang", 
	 "Gill Grunt", "Zap", "Wham-Shell", "Slam Bam", 
	 "Ignitor", "Flameslinger", "Eruptor", "Sunburn",
	 "Boomer", "Trigger Happy", "Drill Sergeant", "Drobot",
	 "Hex", "Chop Chop", "Cynder", "Ghost Roaster",
	 "Whirlwind", "Lightning Rod", "Warnado", "Sonic Boom",
	 "Stump Smash", "Stealth Elf", "Camo", "Zook"
	];
const skylanderSpeciesIds = skylanderSpecies.map(s => s.toLowerCase().replace(" ","").replace("-",""));


function buildUrl(skylanderName: string,imageType: "Transparent_Render" | "Icon" | "Portrait_ssf_") {
	
	return "https://static.wikia.nocookie.net/skylanders/images/" + getFandomImageHashPath(buildFileName(skylanderName.replace(" ","_"), imageType));
}
function buildFileName(skylandername: string, imageType: "Transparent_Render" | "Icon" | "Portrait_ssf_") {
	if(imageType == "Portrait_ssf_"){
		...
		if(skylandername == "Cynder") return "Portrait_cynder_ssf.png"	
		if(skylandername == "Chop_Chop") return "Portrait_ghost_roaster_ssf.png"	
		if(skylandername == "Ghost_Roaster") return "Portrait_chop_chop_ssf.png"
		if(skylandername == "Boomer") return "Portrait_boomer_ssf.png"
		if(skylandername == "Slam_Bam") return "Portrait_slam_bam_ssf.png"	
		if(skylandername == "Gill_Grunt") return "Portrait_gill_grunt_ssf.png"
		if(skylandername == "Stealth_Elf") return "Portrait_sssf_stealth_elf.png"

		return imageType + skylandername.toLowerCase() + ".png";
	}else {
		if(imageType == "Transparent_Render"){
			if(skylandername == "Wham-Shell") return "LightCore_Wham-Shell_Transparent_Render.png"
		}

		return skylandername + "_" + imageType + ".png";
	}
}
...
// Main function to get the image hash path like "1/1a/Filename.png"
function getFandomImageHashPath(filename: string): string {
  const hash = md5(filename);
  const first = hash.charAt(0);
  const firstTwo = hash.substring(0, 2);
  return `${first}/${firstTwo}/${filename}`;
}
...
export const Dex = new class implements ModdedDex {
	...
	getSpriteData(pokemon: Pokemon | Species | string, isFront: boolean, options: {
		...
		//TODO: add skylander files here
		const skylandername = species.name;
		if(skylanderSpecies.includes(skylandername)){
			...
			return {
				...
				//url: getImage(skylandername, "Transparent_Render"),
				//url: "./sprites/custom/"+ skylandername.replace(" ","_") + "/" + dir + ".jpg",
				pixelated: true,
				isFrontSprite: isFront,
				cryurl: 'audio/cries/pikachu.mp3',
				shiny: options.shiny,
			};
		}
	...
	getPokemonIcon(pokemon: string | Pokemon | ServerPokemon | Dex.PokemonSet | null, facingLeft?: boolean) {
		...
		let skylandername = String(pokemon)

		if(skylanderSpecies.includes(skylandername)){
			const urls = buildUrl(skylandername, "Icon")			
			return `background-image: url(${urls});background-size: 25px 25px;background-position:center;background-repeat:no-repeat;`;
			return `pokemon: ${encodeURIComponent(JSON.stringify(pokemon))};background-image: url(${urls});background-size: 25px 25px;background-position:center;background-repeat:no-repeat;`;
		}
		...
		skylandername = id
		if(skylanderSpeciesIds.includes(skylandername) ){
			 const index = skylanderSpeciesIds.findIndex((e) => {
			 	if(e === skylandername) return true
			 })
			 skylandername = skylanderSpecies[index]
			const urls = buildUrl(skylandername, "Icon")			
			return `background:url(${urls});background-position:center;background-size:contain;background-repeat:no-repeat`;
			return `pokemon: ${encodeURIComponent(JSON.stringify(pokemon))};id: ${id};num: ${num};background:url(${urls});background-position:center;background-size:contain;background-repeat:no-repeat`;
		}
		//return ("Pokemon:"+JSON.stringify(pokemon) + " " + "Id"+id + " " + "")
		...
		return `pokemon: ${JSON.stringify(pokemon)};id: ${id};num: ${num};background:transparent url(${Dex.resourcePrefix}sprites/pokemonicons-sheet.png?v19) no-repeat scroll -${left}px -${top}px${fainted}`;
	}
	...
	getTeambuilderSprite(pokemon: any, dex?: ModdedDex, xOffset = 0, yOffset = 0) {
		...
		const skylandername = pokemon.species.trim()
		if(skylanderSpecies.includes(skylandername)){
			const urls = buildUrl(skylandername, "Icon")
			return `--pokemon: ${encodeURIComponent(JSON.stringify(pokemon))};--data: ${encodeURIComponent(JSON.stringify(data))};background-image:url(${urls});background-size:15%;background-position:10px 0px;background-repeat:no-repeat;${resize}`;

			return `background-image:url(${urls});background-size:15%;background-position:10px 0px;background-repeat:no-repeat;${resize}`;
		}
